...
Cacher.prototype.keySQL = function key(sql, options) {
  ...
  if (!options) {
    ...
    hash = crypto.createHash('sha1')
      .update(sql + '-' + CircularJSON.stringify(options, jsonReplacer))
      .digest('hex');
  }
  return [this.cachePrefix, '__raw__', 'query', hash].join(':');
};

Cacher.prototype.key = function key(options, method, model) {
  const hash = crypto.createHash('sha1')
    .update(CircularJSON.stringify(options, jsonReplacer))
    .digest('hex');
  return [this.cachePrefix, model, method, hash].join(':');
};
...
Cacher.prototype.run = function run(options) {
  return this.fetchFromCache(options, this.method, this.modelName);
};
 ...
 */
Cacher.prototype.fetchFromCache = function fetchFromCache(options, method, model) {
  var self = this;
  return new Promise(function promiser(resolve, reject) {
    var key = self.key(options, method, model);
